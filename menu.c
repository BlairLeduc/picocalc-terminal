...
void menu(void)
{
    ...
    {
        printf("\033[%d;8H%s: \033[%d;24H\033[%dm%6s\033[m",
    ...
    while (ch != '\n' && ch != '\r')
    {
        ...
        if (ch == KEY_LEFT)
        {
            ...
            if (menu_items[item].selected > 0)
            {
                *menu_items[item].value = menu_items[item].values[--menu_items[item].selected].value;
                printf("\033[%d;24H\033[4m%6s\033[m", 8 + item * 2, menu_items[item].values[menu_items[item].selected].name);
            }
        }
        ...
        else if (ch == KEY_UP)
        {
            ...
            if (item > 0)
            {
                printf("\033[%d;24H%6s", 8 + item * 2, menu_items[item].values[menu_items[item].selected].name);
                item--;
                printf("\033[%d;24H\033[4m%6s\033[m", 8 + item * 2, menu_items[item].values[menu_items[item].selected].name);
            }
        }
